...
{
    public class RoleLookupController : Controller, ILookupController
    {
        protected readonly WebAppContext _context;
        protected const int PageSize = 7;

        public RoleLookupController(WebAppContext context)
        {
            _context = context;
        }
        ...
        {
            var owner = _context.Roles.Single(x => x.Id == v) ?? new Role();
            return Json(new KeyContent(owner.Id, owner.Name));
        }

        public virtual ActionResult Search(int page, string value = "")
        {
            var contacts = _context.Roles.Where(x => x.Name.ToLower().Contains(value.ToLower().Trim()));
